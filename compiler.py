...
if __name__ == "__main__":
    ...
    data_loaded = yaml.safe_load(open(args.yaml, 'r'))
    ...
    for file in list(split_s.keys()):
        ...
        png_to_binary.whatIs = data_loaded["type"]
            ...
            for y in range(newHeight):
                for x in range(newWidth):
                    if i >= len(list(unique_tiles.keys())):
                        break
                    s = unique_tiles[list(unique_tiles.keys())[i]]
                    a = (x*8, y*8, (x+1)*8, (y+1)*8)
                    newImage.paste(s, a)
                    i += 1

            newImage.save("TESTEST.png")

        else:
            newImage = png_to_binary.PngToTilePng(image_path)

        file_bytes = png_to_binary.PngTo2bpp(newImage, use_style, assumed_palettes)
    ...
    open(args.yaml.replace(".yaml", ".bin"), "wb").write(sheet_bytes)
